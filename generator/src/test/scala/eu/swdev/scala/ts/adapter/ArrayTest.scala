...
class ArrayTest extends AdapterFunSuite {
  ...
  """
    ...
    |      def x = _root_.adapter.ArrayTest.x.$cnv[_root_.eu.swdev.scala.ts.tps.ReadOnlyArray[Int]]
    |      def x_=(value: _root_.eu.swdev.scala.ts.tps.ReadOnlyArray[Int]) = _root_.adapter.ArrayTest.x = value.$cnv[_root_.scala.Array[Int]]
    |    }
    |  }
    |}
    |""".check()
